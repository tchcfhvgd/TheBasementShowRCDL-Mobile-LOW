...
class FlxAnim implements IFlxDestroyable
{
	...
	**/
	...
	public function play(?Name:String = "", ?Force:Bool = false, ?Reverse:Bool = false, ?Frame:Int = 0)
	{
		...
		if (Name != null && Name != "")
		{
			...
			if (curThing == null)
			{
				...
				if (symbol != null) {
					trace(symbol.name == curSymbol.name);
					curThing = {
	...
	function setSymbols(Anim:AnimAtlas)
	{
		...
		//trace("> Parse Stage Symbol", (Timer.stamp() - t0));

		//var matrixTime:Float = 0;
		if (Anim.SD != null)
		{
			for (symbol in Anim.SD.S)
			{
				//FlxElement.matrixTime = 0;
				//var t0 = Timer.stamp();
				symbolDictionary.set(symbol.SN, new FlxSymbol(symbol.SN, FlxTimeline.fromJSON(symbol.TL)));
				//trace('> Parse ${symbol.SN} Symbol', (Timer.stamp() - t0));
				//trace('> Parse ${symbol.SN} Matrix', FlxElement.matrixTime);
				//matrixTime += FlxElement.matrixTime;
			}
		}
		//trace('> Total Matrix Time', matrixTime);
	}
