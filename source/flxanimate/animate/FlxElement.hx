...
{
	//public static var matrixTime:Float = 0;
	...
	//static var matrixNames = ["m00","m01","m02","m03","m10","m11","m12","m13","m20","m21","m22","m23","m30","m31","m32","m33"];
	...
	public static function fromJSON(element:Element)
	{
		...
		/*m = if((m3d is Array)) {
			...
			//var array = ["m00","m01","m02","m03","m10","m11","m12","m13","m20","m21","m22","m23","m30","m31","m32","m33"];//Reflect.fields(m3d);
			//array.sort((a, b) -> Std.parseInt(a.substring(1)) - Std.parseInt(b.substring(1)));

			//[for (field in array) Reflect.field(m3d,field)];
			[for (field in matrixNames) Reflect.field(m3d,field)];
		}*/
		m = if((m3d is Array)) {
            [m3d[0], m3d[1], m3d[4], m3d[5], m3d[12], m3d[13]];
        } else {
            //var array = ["m00","m01","m10","m11","m30","m31"];

            [for (field in matrixNames) Reflect.field(m3d,field)];
        }

		//trace("Matrix code", (Timer.stamp() - t0));
		//FlxElement.matrixTime += (Timer.stamp() - t0);
		...
		return new FlxElement(
			...
			params,
			//new FlxMatrix(m[0], m[1], m[4], m[5], m[12] + pos.x, m[13] + pos.y)
