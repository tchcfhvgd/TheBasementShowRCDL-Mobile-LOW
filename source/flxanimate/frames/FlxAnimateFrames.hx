...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function fromTextureAtlas(Path:String):FlxAtlasFrames
	{
		...
		if (zip != null || haxe.io.Path.extension(Path) == "zip")
		{
			...
			var thing = (zip != null) ? zip : Zip.unzip(Zip.readZip(Paths.getBytes(Path)));
			for (list in thing)
			{
				var extension = haxe.io.Path.extension(list.fileName);
				if (extension == "json")
				{
		...
		{
			function addSpriteMap(curJson:AnimateAtlas, curSpritemap:FlxGraphic) {
			...
			if (Paths.exists('$Path/spritemap.json'))
			{
				var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(Paths.getText('$Path/spritemap.json'), String.fromCharCode(0xFEFF), ""));
				if(curJson.meta.image.endsWith(".png")) {
					curJson.meta.image = curJson.meta.image.substring(0, curJson.meta.image.length - 4);
				}
				var curSpritemap = Paths.image('$Path/${curJson.meta.image}', null, "");
				//trace(Path, curJson.meta.image);
				addSpriteMap(curJson, curSpritemap);
			}
			var i = 1;
			while (Paths.exists('$Path/spritemap$i.json'))
			{
				var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(Paths.getText('$Path/spritemap$i.json'), String.fromCharCode(0xFEFF), ""));
				if(curJson.meta.image.endsWith(".png")) {
					curJson.meta.image = curJson.meta.image.substring(0, curJson.meta.image.length - 4);
				}
				var curSpritemap = Paths.image('$Path/${curJson.meta.image}', null, "");
	...
	// TODO: Fix edge on limbs not being anti aliased
	static function textureAtlasHelper(SpriteMap:BitmapData, limb:AnimateSpriteData, curMeta:Meta)
	{
		...
		@:privateAccess
		var curFrame = new FlxFrame(FlxG.bitmap.add(SpriteMap));
		curFrame.name = limb.name;
		curFrame.sourceSize.set(width, height);
		//trace(limb.name, limb.x, limb.y, width, height);
		var padding:Float = 1;
		...
		//curFrame.frame = new FlxRect(limb.x, limb.y, width, height);
		if(limb.rotated) {
		...
		var width = (limb.rotated) ? limb.h : limb.w;
		var height = (limb.rotated) ? limb.w : limb.h;
		var sprite = new BitmapData(width + PADDING*2, height + PADDING*2, true, 0);
		...
		matrix.tx += PADDING;
		matrix.ty += PADDING;
		sprite.draw(SpriteMap, matrix, null, null, new Rectangle(PADDING, PADDING, width, height), true);

		@:privateAccess
		var curFrame = new FlxFrame(FlxG.bitmap.add(sprite));
		curFrame.name = limb.name;
		curFrame.sourceSize.set(width, height);
		curFrame.frame = new FlxRect(PADDING, PADDING, width, height);
		return curFrame;*/
